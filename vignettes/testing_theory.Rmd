...
```{r}
prop3 |> filter(id %in% c(511,635,317,645))
```
...
propositions
ggsave(here::here("graphs", "test_of_propositions.pdf"), plot=propositions, 
       width=7.29, height = 4.5, units="in")
```
...
ggsave(here::here("graphs", "power_analysis.pdf"), width=7.29, height=4.5, units="in")
```
...
power %>% gt::gt() |> gt::fmt_number(decimals=3)
```
