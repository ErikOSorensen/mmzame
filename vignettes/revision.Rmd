...
$$ Can then test the hypothesis that $\alpha$ (weight on $y$, money for self) is equal to $1/2$. But this involves much more 
parametric structure than we use in the rest of the paper and behavior is not linearly dependent on the CES parameters,
making them a bit difficult to interpret.
...
Our main criterion for symmetry is that the share of tokens for self in the
...
All cases, but for the symmetric/non-symmetric (our simple criterion):
...
all_tests_symmetric <- list(prop3_ps, prop2_ps, prop4_ps) %>% 
  bind_rows() %>% left_join(self_shares) %>% mutate(symmetric = abs(mean_share_self-0.5)<0.05,
                                                    symmetricf = factor(ifelse(symmetric,"Symmetric","Not symmetric")))
all_tests_symmetric %>% ggplot(aes(x=p, y = (..count..)/tapply(..count..,..PANEL..,sum)[..PANEL..])) +
  geom_histogram(binwidth=0.05, boundary=0) +
  theme_minimal() +
  facet_grid(comparison~symmetricf) +
  labs( y = "Fraction of participants",
        x = "P-value on H0: Equality of preferences",
        title = "Testing equality of preferences by symmetry (simple criterion)",
        caption="Binwidth: 0.05.")
```
...
ggplot(aes(x=p, y = (..count..)/tapply(..count..,..PANEL..,sum)[..PANEL..])) +
  ...
  labs( y = "Fraction of participants",
        x = "P-value on H0: Equality of preferences",
        title = "Testing equality of preferences by symmetry (alternative criterion)",
        caption="Binwidth: 0.05.")
```
...
allp_with_symmetry_df |> ggplot(aes(x = p_com, 
  ...
  labs(x = "P-value on H0: Equality of preferences",
       y = "Fraction of participants",
       title ="Testing equality across all 3 domains",
       caption="Binwidth: 0.05. Alternative definition of symmetric.") +
  theme(plot.title.position = "plot") +
...
allp_with_simple_symmetry_df <- allp_df |> 
  mutate(symmetricf = factor(ifelse(id %in% symmetric_subset$id, "Symmetric", "Not symmetric")))
allp_with_simple_symmetry_df |> ggplot(aes(x = p_com, 
  ...
  labs(x = "P-value on H0: Equality of preferences",
       y = "Fraction of participants",
       title ="Testing equality across all 3 domains",
       caption="Binwidth: 0.05. Simple definition of symmetric.") +
  theme(plot.title.position = "plot") +
