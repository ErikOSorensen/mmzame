...
tar_load(p3_00m, store = here::here("_targets"))
tar_load(p4_00m, store = here::here("_targets"))
tar_load(p2_00m, store = here::here("_targets"))
tar_load(pall_domainsm, store = here::here("_targets"))
...
allp3m <- map(p3_00m, p_from_list) |> bind_rows() |> mutate(tst="mSRISK = mPRISK")
allp4m <- map(p4_00m, p_from_list) |> bind_rows() |> mutate(tst="mSRISK = mSOCIAL")
allp2m <- map(p2_00m, p_from_list) |> bind_rows() |> mutate(tst="mSOCIAL = mPRISK")
alldm <- map(pall_domainsm, p_from_list) |> bind_rows() |> mutate(tst="mALL DOMAINS")
...
```{r}
psm <- c("impartial05","stochastic_symmetricd10","stochastic_symmetricd05","stochastic_symmetricd01")
all_numrows_with_symmetry <- main_rows(all,
                                       c("impartial05","stochastic_symmetricd10","stochastic_symmetricd05","stochastic_symmetricd01"),
                                       cnames = c("p_mSRISK = mPRISK", "p_mSRISK = mSOCIAL", "p_mSOCIAL = mPRISK", "p_mALL DOMAINS"))
```
...
Variant of output table B that impose symmetry:
