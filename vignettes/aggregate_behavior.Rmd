...
average_social_decisions %>% ggplot(aes(x=mean_yshare, y=(..count..)/tapply(..count..,..PANEL..,sum)[..PANEL..])) + 
  ...
  theme_minimal() + theme( text=element_text(family="Roboto") )
ggsave(here::here("graphs", "aggregate_social_choice.pdf"))
embed_fonts(here::here("graphs", "aggregate_social_choice.pdf"))
```
...
average_symmetric_decisions %>% ggplot(aes(x=mean_cheapest, y=(..count..)/tapply(..count..,..PANEL..,sum)[..PANEL..])) +
  ...
  labs(x = "Mean share of cheapest asset in portfolio",
       y = "Fraction") +
  theme_minimal() + theme( text=element_text(family="Roboto") ) + facet_grid(.~treatmentf)
ggsave(here::here("graphs", "aggregate_risks.pdf"))
embed_fonts(here::here("graphs", "aggregate_risks.pdf"))
```
...
bar_fractions %>% ggplot(aes(x=ygroup, y=fraction, group=treatmentf, fill=treatmentf)) + 
  ...
  labs(x = "Share to self\u2014or to the cheapest asset",
       y = "Fraction") +
  theme_minimal() + 
  theme(legend.title=element_blank(), text=element_text(family="Roboto")) + 
  scale_fill_grey()
ggsave(here::here("graphs","aggregate_choices.pdf"))
embed_fonts(here::here("graphs","aggregate_choices.pdf"))
```
