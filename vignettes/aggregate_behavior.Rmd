...
Another "Rawlsian" way of thinking is that the difference in what is allocated to x and y is very small,

```{r}
mmzame_decisions |> filter(treatment=="dictator") |>
  mutate(reldiff = abs(x-y)/(x+y),
         selfshare = y/(x+y)) |>
  group_by(id) |>
  summarize(max_reldiff = max(reldiff),
            mean_reldiff = mean(reldiff),
            max_selfshare = max(selfshare),
            min_selfshare = min(selfshare)) |>
  arrange(mean_reldiff) |>
  head() |> 
  gt::gt()
```
...
ggsave(here::here("graphs","aggregate_choices.pdf"), width = 16, height = 10, units = "cm", device = cairo_pdf)
ggsave(here::here("graphs","aggregate_choices.eps"), width = 16, height = 10, units = "cm", device = cairo_ps)
