...
of the test statistic changes, but in general the minimum and maximum of K draws
from a permutation distribution $\widehat{F}$ can be defined
as 
$$ \widehat{F}_{e^{+}}(s) = \left(\widehat{F}(s)\right)^K,$$
and 
$$
\widehat{F}_{e^{-}}(s) =  1 - \left(1 - \widehat{F}(s)\right)^K.
$$
...
implement with a small $\epsilon$ offset because the permutation distributions of $\widehat{F}$
are lumpy.  I have coded this up as a general function that takes a list of choice data
frames as an argument, and which takes care of learning K from the length of this list.
This function is not as fast as purpose-built 2-domain functions, and so far I haven't 
added the option of adding "Bronars" choices. 

Loading pre-calculated tests, we can look at the distribution of p-values in the test
